...
var races_beastfree_darkelf_free = ['Human','Elf','Dark Elf','Orc','Ogre','Giant','Gnome','Goblin','Demon','Dragonkin','Kobold','Lizardfolk','Fairy','Seraph','Dryad','Lamia','Harpy','Arachna','Nereid','Scylla','Slime']
...
func setRace(person,mother):
	...
	if caprace in humanoid_races_array:
		...
		if person.genealogy.horse >= highestpercent: #ralph3 - should fix instances of <50% Centaur dominant hybrids being born as 'Halfkin ' only for race
			...
			person.race = 'Taurus'
		elif person.genealogy.hyena >= highestpercent:
			person.race = 'Gnoll'
		else:
